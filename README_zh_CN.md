...
- **独立图标选择器**：
  ...
  - 最近使用；
  - 加载更多 / 分页。
- **图标集偏好**：在设置里用复选框指定只使用的 Iconify 图标集（支持搜索 / 全选 / 清空 / 反选），默认已勾选几个常用彩色图标集，未勾选表示全部。原生面板、独立选择器与自动匹配都会在选定范围内搜索。
...
| 使用的图标集 | 勾选只在这些 Iconify 图标集中搜索；一个都不勾选表示全部 | 默认 7 个常用彩色图标集 |

设置页底部还有一个「**重置设置**」按钮，可把所有设置恢复为默认值；点击后会先弹出确认框。
